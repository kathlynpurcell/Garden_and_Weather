# -*- coding: utf-8 -*-
"""
Created on Mon Jun 15 21:22:45 2020
...
#finding the day to use as a label
table_n = soup.find_all('time-layout')
name = soup.find_all('start-valid-time')

day_final = []
for i in range (len(name)):
    day_index = str(name[i])
    day_inside = day_index[day_index.find('"')+1:day_index.find('>')-1]
    day_final.append(day_inside)
day_final = day_final[:14] # we only want the first 14 items to match the predicted
day_dict = dict(enumerate(day_final))
...
class GardenWindow(QMainWindow):
    def __init__(self):
        ...
        gridLayout.addWidget(title2, 1, 0)
        ...
        gridLayout.addWidget(title3, 2, 0)
        
        title4 = QLabel('Here is the forcast for the next week. Please plan ahead.' , self) 
        title4.setAlignment(QtCore.Qt.AlignCenter)
        gridLayout.addWidget(title4, 3, 0)
        

        self.graphWidget = pg.PlotWidget()
        self.ax = pg.PlotWidget()
        #self.setCentralWidget(self.graphWidget)
        self.graphWidget.plot(list(day_dict.keys()), pred_final)
        self.graphWidget.resize(500,500)
        self.graphWidget.setLabel('left', text='Pecrect Chance of Rain')
        self.graphWidget.setLabel('bottom', text='Upcoming Day of the Week')
        xax = self.graphWidget.getAxis('bottom')
        xax.setTicks([day_dict.items()])
        gridLayout.addWidget(self.graphWidget, 4, 0)

        
        title5 = QLabel('Thanks for using the program!', self) 
        title5.setAlignment(QtCore.Qt.AlignCenter)
        gridLayout.addWidget(title5, 5, 0)
